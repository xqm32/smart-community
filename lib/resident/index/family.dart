...
class _ResidentFamilyState extends State<ResidentFamily> {
  ...
  void initState() {
    ...
    if (widget.recordId != null) {
      pb.collection('families').getOne(widget.recordId!).then(_setRecord);
    }
    super.initState();
  }
      ...
      body: Stepper(
        ...
        steps: [
          Step(
            ...
            content: _familyForm(index: 0),
          ),
          Step(
            isActive: _index >= 1,
            title: const Text('物业审核'),
            content: _familyForm(index: 1),
          ),
          Step(
            isActive: _index >= 2,
            title: const Text('审核通过'),
            content: _familyForm(index: 2),
          )
        ],
      ),
    );
          ...
          builder: (context) {
            return AlertDialog(
              ...
              actions: <Widget>[
                ...
                TextButton(
                  onPressed: () {
                    pb.collection('families').delete(_record!.id).then((value) {
  ...
  // 居民端/首页/家人管理/填写信息
  Widget _familyForm({required int index}) {
    return Form(
      key: _formKeys[index],
      child: Column(
        children: [
